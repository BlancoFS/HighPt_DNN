...
if __name__ == '__main__':
    ...
    hits = pd.read_csv('data_' + args.kind + '/output_Hit_' + args.FileNumber + '.txt', sep="\t", header=None)
    ...
    props = pd.read_csv('data_' + args.kind + '/output_Prop_' + args.FileNumber + '.txt', sep="\t", header=None)
    ...
    output_mean.to_csv('CLEANED_SEGMENTS_' + args.kind + '/cleaned_mean_' + args.FileNumber + '.csv', index=False)
    ...
    # CREATE AND SAVE IMAGES FROM HITS
    
    for event in data.Hit_Eventid.unique():
        for lumi in data[(data.Hit_Eventid == event)].Hit_EventluminosityBlock.unique():
            for muon in data[(data.Hit_Eventid == event) & (data.Hit_EventluminosityBlock == lumi)].Hit_Muonid.unique():
                df_hit = data[data.Hit_Eventid==event]
                df_hit.loc[:, 'Hit_phi'] = np.arctan(df_hit['Hit_y']/df_hit['Hit_x']) # Use phi instead of (x,y)
                
                # Choose the first station with hits as the origin of the reference frame
                
                if(len(df_hit[df_hit.Hit_DTstation==1].Hit_phi.to_numpy())!=0):
                    mean_phi_first_hit = df_hit[df_hit.Hit_DTstation==1].Hit_phi.mean()
                    mean_z_first_hit = df_hit[df_hit.Hit_DTstation==1].Hit_z.mean()
                elif(len(df_hit[df_hit.Hit_DTstation==2].Hit_phi.to_numpy())!=0):
                    mean_phi_first_hit = df_hit[df_hit.Hit_DTstation==2].Hit_phi.mean()
                    mean_z_first_hit = df_hit[df_hit.Hit_DTstation==2].Hit_z.mean()
                elif(len(df_hit[df_hit.Hit_DTstation==3].Hit_phi.to_numpy())!=0):
                    mean_phi_first_hit = df_hit[df_hit.Hit_DTstation==3].Hit_phi.mean()
                    mean_z_first_hit = df_hit[df_hit.Hit_DTstation==3].Hit_z.mean()
                elif(len(df_hit[df_hit.Hit_DTstation==4].Hit_phi.to_numpy())!=0):
                    mean_phi_first_hit = df_hit[df_hit.Hit_DTstation==4].Hit_phi.mean()
                    mean_z_first_hit = df_hit[df_hit.Hit_DTstation==4].Hit_z.mean()
                    
                df_hit.loc[:, 'Hit_phi'] = df_hit['Hit_phi'] - mean_phi_first_hit
                df_hit.loc[:, 'Hit_z'] = df_hit['Hit_z'] - mean_z_first_hit
                
                # Parameters that select the reference frame size
                
                maxphi = 6.4/10  # we take 1/10 the size of the detector
                maxz = 1400/10
                znbin = 256      # The images are 256x256 pixels
                phinbin = 256
                
                half_phi = phinbin/2
                half_z = znbin/2
                
                # Transform phi and z to coordinates in the matrices
                df_hit['Hit_phicoor'] = df_hit.Hit_phi//(maxphi/phinbin) + half_phi
                df_hit['Hit_zcoor'] = df_hit.Hit_z//(maxz/znbin) + half_z
                
                # Remove points out of the matrix size
                df_hit = df_hit[abs(df_hit['Hit_zcoor'])<256]
                df_hit = df_hit[abs(df_hit['Hit_phicoor'])<256]
                df_hit = df_hit[abs(df_hit['Hit_zcoor'])>0]
                df_hit = df_hit[abs(df_hit['Hit_phicoor'])>0]
                
                # Four matrices, one per station
                MB1 = np.zeros((phinbin, znbin))
                MB2 = np.zeros((phinbin, znbin))
                MB3 = np.zeros((phinbin, znbin))
                MB4 = np.zeros((phinbin, znbin))
                
                phi_coor = df_hit.Hit_phicoor.to_numpy()
                z_coor = df_hit.Hit_zcoor.to_numpy()
                DT_st = df_hit.Hit_DTstation.to_numpy()
    ...
    output.to_csv('STEP1_' + args.kind + '/output' + args.FileNumber + '.csv', index=False)
