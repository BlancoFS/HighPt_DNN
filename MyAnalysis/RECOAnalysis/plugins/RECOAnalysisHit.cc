...
class RECOAnalysis : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      edm::InputTag theDTRecHitLabel, theCSCRecHitLabel,inputMuonShowerInformationValueMap;
      edm::EDGetTokenT<DTRecHitCollection> dtRecHitsToken;
      edm::EDGetTokenT<CSCRecHitCollection> cscRecHitsToken;
...
RECOAnalysis::RECOAnalysis(const edm::ParameterSet& iConfig)
{
   ...
   // theDTRecHitLabel = iConfig.getParameter<edm::InputTag>("segmentsDt");
   // theCSCRecHitLabel = iConfig.getParameter<edm::InputTag>("segmentsCSC");
   theDTRecHitLabel = iConfig.getParameter<edm::InputTag>("DTRecSegmentLabel"); // Check if correct label
   theCSCRecHitLabel = iConfig.getParameter<edm::InputTag>("CSCRecSegmentLabel");
   dtRecHitsToken = consumes<DTRecHitCollection>(theDTRecHitLabel);
   cscRecHitsToken = consumes<CSCHitCollection>(theCSCRecHitLabel);
...
void RECOAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   iEvent.getByToken(dtRecHitsToken, dtRecHits);

   edm::Handle<CSCRecHitCollection> cscRecHits;
   iEvent.getByToken(cscRecHitsToken, cscRecHits);
   ...
   for (auto itmuon=muons->begin(); itmuon != muons->end(); itmuon++){
     ...
     std::map<const GeomDet*, std::vector<TrackingRecHit *> > DetAllHitsMap; 
     ...
     for (auto itHit = dtRecHits->begin(); itHit != dtRecHits->end(); itHit++) {
       ...
       std::map<const GeomDet*, std::vector<TrackingRecHit *> >::iterator it = DetAllHitsMap.find(geomDet);

       if(it == DetAllHitsMap.end()) {
	        ...
	        DetAllHitsMap.insert(std::pair<const GeomDet*, std::vector<TrackingRecHit *> > (geomDet, trhit));
       } else {
     ...
     for(auto it = DetAllHitsMap.begin(); it != DetAllHitsMap.end(); it++) {
