...
# MLP:
model1.add(layers.Dense(int(sys.argv[1]), activation='relu',input_dim=len(variablesTrain))) #, kernel_regularizer=l2(0.001)
...
history = model.fit(train[variablesTrain],train[genpT],validation_split=0.1, epochs=1000, batch_size=int(sys.argv[4]), verbose=0, callbacks=[callbacks.EarlyStopping(monitor='val_loss',patience=50,verbose=1)])
